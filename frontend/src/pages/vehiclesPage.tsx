    ...
    try {
      ...
      toast.error(
        [error.response?.data?.message, error.response?.data?.errors?.map((e: any) => e.message).join(", ")]
          .filter(Boolean)
          .join(", ") || "Error al guardar el vehículo",
      )
    }
  }
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      toast.error(
        [error.response?.data?.message, error.response?.data?.errors?.map((e: any) => e.message).join(", ")]
          .filter(Boolean)
          .join(", ") || "Error al eliminar el vehículo",
      )
    }
  }
  ...
  return (
    <div className="p-6">
      ...
      <Card className="bg-card">
        ...
        <CardContent>
          {loading ? (
            <div className="text-center py-4 text-muted-foreground">Cargando vehículos...</div>
          ) : (
          ...
          {modalType === "delete" ? (
            <>
              ...
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={closeModal} className="bg-background text-foreground">
                  Cancelar
                </Button>
                <Button
                  variant="destructive"
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm mb-1 text-foreground">Placa</label>
                  <Input
                    type="text"
                    value={licensePlate}
              ...
              <div className="flex justify-end gap-2 mt-4">
                <Button type="button" variant="outline" onClick={closeModal} className="bg-background text-foreground">
                  Cancelar
                </Button>
